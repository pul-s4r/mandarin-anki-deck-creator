"""Shared StateStore conformance tests (subclass in backend-specific test modules)."""
...
class StateStoreConformanceTests:
    """Mixin-style test suite; subclasses must provide a module-level ``store`` fixture."""

    def test_get_source_record_respects_user_id(self, store: StateStore) -> None:
    ...
    def test_roundtrip_source_chunk_card_run(self, store: StateStore) -> None:
        now = datetime.now(UTC)
        src = SourceRecord(
            source_id="sid1",
            provider="local-filesystem",
    ...
    def test_iter_cards_changed_since(self, store: StateStore) -> None:
    ...
    def test_concurrent_upserts(self, store: StateStore) -> None:
    ...
    def test_upsert_updates_ankiweb_when_vocab_unchanged(self, store: StateStore) -> None:
        now = datetime.now(UTC)
        cr = CardRecord(
            card_id="c1",
            simplified="词",
